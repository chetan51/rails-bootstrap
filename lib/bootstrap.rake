...
namespace :bootstrap do
  ...
  task :install, :dependency do |t, args|
    ...
    when "spine"
      Rake::Task["bootstrap:install_spine"].invoke
    when "backbone"
      Rake::Task["bootstrap:install_backbone"].invoke
    when "columnal"
      Rake::Task["bootstrap:install_columnal"].invoke
    when "columnal_without-typography"
      Rake::Task["bootstrap:install_columnal_without_typography"].invoke
    when "twitter_bootstrap"
      Rake::Task["bootstrap:install_twitter_bootstrap"].invoke
    when "twitter_bootstrap_without-grid"
      Rake::Task["bootstrap:install_twitter_bootstrap_without_grid"].invoke
    end
  end
...
task :install_twitter_bootstrap => ["app/assets/stylesheets/include"] do
  ...
  `wget http://twitter.github.com/bootstrap/1.4.0/bootstrap.min.css -O tmp/twitter_bootstrap.min.css`
